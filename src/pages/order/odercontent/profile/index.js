...
export default function Profile(props) {
  ...
  async function handleSubmit(e) {
    ...
    if (!completeName || !phoneNumber) {
      ...
      if (street !== null && street !== undefined && street.length > 60) {
        dispatch(setErrors("Endereço inválido"));
      }

      if (placeNumber !== null && placeNumber !== undefined && placeNumber > 200000) {
      ...
      if (neighborhood !== null && neighborhood !== undefined && neighborhood.length > 650) {
        dispatch(setErrors("Bairro inválido"));
      }

      if (cep !== null && cep !== undefined && cep.length > 10) {
      ...
      if (state.data.address.state !== null && state.data.address.state !== undefined && state.data.address.state.length > 2) {
